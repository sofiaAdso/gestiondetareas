...
public class UsuarioDao {
    ...
    public Usuario validar(String user, String pass) {
        // ✅ CORRECCIÓN: trim() para evitar espacios accidentales
        ...
        try (Connection con = Conexion.getConexion()) {
            if (con == null) {
                System.err.println("❌ validar(): No se pudo obtener conexión a la BD.");
                return null;
            }
            try (PreparedStatement ps = con.prepareStatement(sql)) {
                ps.setString(1, user);
                ps.setString(2, pass);
                System.out.println(">> Ejecutando validación para usuario: " + user);
                try (ResultSet rs = ps.executeQuery()) {
                    if (rs.next()) {
                        ...
                        System.out.println("⚠️ No se encontró usuario con esas credenciales.");
                    }
                }
            }
        } catch (SQLException e) {
            System.err.println("❌ Error en Login DAO: " + e.getMessage());
            LOGGER.log(Level.SEVERE, "Error SQL en validar()", e);
        }
        return null;
    }
